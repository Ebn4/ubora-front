...
export class PeriodCandidaciesPreselectedComponent extends BaseListWidget {
  ...
  exportToExcel() {
    ...
    this.candidacyService.getPreselectedCandidates(1, '', 'all', this.period.id)
      .subscribe({
        next: (response) => {
          ...
          XLSX.utils.book_append_sheet(workbook, worksheet, "Présélectionnés");
          const excelBuffer = XLSX.write(workbook, {
            bookType: 'xlsx',
            type: 'array'
          });
          const fileName = `candidats_Preselectionnes_periode_${this.period?.year}.xlsx`;
